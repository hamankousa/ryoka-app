...
describe("ListTabScreen download", () => {
  it("shows download button and invokes download service", async () => {
    const ListTabScreen = require("../app/(tabs)/list").default;
    const { downloadService } = require("../src/features/download/downloadService");
    render(<ListTabScreen />);

    await waitFor(() => {
      expect(screen.getByText("都ぞ弥生")).toBeTruthy();
    });

    fireEvent.press(screen.getByText("DL"));
    expect(downloadService.downloadSong).toHaveBeenCalledTimes(1);
  });
});
